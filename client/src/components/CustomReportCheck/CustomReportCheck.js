...
import {updateCustomReport} from "../../actions/reportActions"
import { connect } from "react-redux";
const uuidv1 = require("uuid/v1");

export class CustomReportCheck extends Component {
  constructor(props) {
    super(props);
  ...
  componentWillReceiveProps(props) {
    ...
    if (projects) {
      ...
      this.setState({ projects: [], visible: false });
    }
  }
  validate = () => {
    const verifiedReport = {...this.props.customReport, verified: true}
    this.props.updateCustomReport(verifiedReport);
  }
  ...
  render() {
    ...
    const tableData = this.state.projects.map(project => {
      ...
      return [<tr>{firstRow}</tr>, nextRows.map(row => row)];
    });

    const table = this.state.visible ? (
      <table>
        <tbody>
          <tr>
    ...
    return <div>{validateButton}{table}</div>;
  }
}

export default connect(null, {updateCustomReport})(CustomReportCheck);